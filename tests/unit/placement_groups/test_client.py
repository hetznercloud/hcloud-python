...
class TestBoundPlacementGroup(object):
    ...
    def bound_placement_group(self, hetzner_client):
        return BoundPlacementGroup(client=hetzner_client.placement_groups, data=dict(id=897))

    def test_bound_placement_group_init(self, placement_group_response):
        bound_placement_group = BoundPlacementGroup(
            client=mock.MagicMock(),
            data=placement_group_response['placement_group']
        )

        check_variables(bound_placement_group, placement_group_response['placement_group'])

    def test_update(self, hetzner_client, bound_placement_group, placement_group_response):
        hetzner_client.request.return_value = placement_group_response
        placement_group = bound_placement_group.update(name=placement_group_response['placement_group']['name'], labels=placement_group_response['placement_group']['labels'])
        hetzner_client.request.assert_called_with(url="/placement_groups/{placement_group_id}".format(placement_group_id=placement_group_response['placement_group']['id']), method="PUT",
                                                  json={"labels": placement_group_response['placement_group']['labels'], "name": placement_group_response['placement_group']['name']})
    ...
    def test_delete(self, hetzner_client, bound_placement_group):
        ...
        hetzner_client.request.assert_called_with(url="/placement_groups/897", method="DELETE")
...
class TestPlacementGroupsClient(object):
    ...
    def test_get_by_id(self, placement_groups_client, placement_group_response):
        ...
        placement_group = placement_groups_client.get_by_id(placement_group_response['placement_group']['id'])
        placement_groups_client._client.request.assert_called_with(url="/placement_groups/{placement_group_id}".format(placement_group_id=placement_group_response['placement_group']['id']), method="GET")
    ...
    def test_get_list(self, placement_groups_client, two_placement_groups_response, params):
        placement_groups_client._client.request.return_value = two_placement_groups_response
        ...
        assert len(placement_groups) == len(two_placement_groups_response['placement_groups'])

        for placement_group, expected in zip(placement_groups, two_placement_groups_response['placement_groups']):
            assert placement_group._client is placement_groups_client
    ...
    def test_get_all(self, placement_groups_client, two_placement_groups_response, params):
        placement_groups_client._client.request.return_value = two_placement_groups_response
    ...
    def test_get_by_name(self, placement_groups_client, one_placement_group_response):
        placement_groups_client._client.request.return_value = one_placement_group_response
        placement_group = placement_groups_client.get_by_name(one_placement_group_response['placement_groups'][0]['name'])
